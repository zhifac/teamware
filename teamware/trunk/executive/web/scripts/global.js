...
function workflowTemplateQuitConfirmation() {
	var answer = confirm("Leave Workflow Template Configuration?")
	if (answer){
		window.location.href = window.location.href.substring(0,location.href.lastIndexOf('/')) + "/projects.html?method=listAll";
	}	
}
...
function popupBox(url, title){
	newWindow = window.open(url,title, "location=0,status=1,width=650,height=500,scrollbars=1,menubar=0,toolbar=0,resizable=1");
  ...
  function anyChecked(theForm) { // check all the checkboxes in the list
  var found = false;
  ...
  function setMasterCheckbox(theForm,name) { // check all the checkboxes in the list
	  var flag = true;

	  for (var i=0;i<theForm.elements.length;i++) {
	    var e = theForm.elements[i];
	    var eName = e.name;
	     if ((e.type.indexOf("checkbox") == 0) && e.name!= name && !e.disabled) {
	         flag = false;
	     }
	  }
	  theForm.elements[name].disabled = flag;
	 }
