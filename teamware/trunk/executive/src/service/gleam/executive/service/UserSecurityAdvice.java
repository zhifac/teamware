...
public class UserSecurityAdvice implements MethodBeforeAdvice,
  ...
  /**
   * Method to enforce security and only allow administrators to modify
   * users. Regular users are allowed to modify themselves.
   */
  public void before(Method method, Object[] args, Object target)
    ...
    if(ctx.getAuthentication() != null) {
      ...
      for(int i = 0; i < roles.length; i++) {
        if(roles[i].getAuthority().equals(Constants.ADMIN_ROLE) || roles[i].getAuthority().equals(Constants.SUPERADMIN_ROLE)) {
          administrator = true;
          break;
        }
      }
