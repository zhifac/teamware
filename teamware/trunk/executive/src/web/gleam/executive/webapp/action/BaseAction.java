...
import gleam.executive.util.CurrencyConverter;
import gleam.executive.util.DateConverter;
import gleam.executive.util.NullConverter;

import org.springframework.context.ApplicationContext;
import org.springframework.web.context.support.WebApplicationContextUtils;

/**
 * Implementation of <strong>Action</strong> that contains base methods
 * for Actions as well as determines with methods to call in subclasses.
 * This class uses the name of the button to determine which method to
 ...
 * 
 * <p>
 * The name of the button is set with the property parameter, i.e., the
 * name of the first button is method.findName. The name of the second
 * button is method.findEmail.
 * </p>
 * 
 * <p>
 * As per HTML/HTTP, whatever submit button that is pushed causes only
 * that submit button's name to be sent as a request parameter to the
 * action.
 * </p>
 * 
 * <p>
 * This action looks for the name by removing the prepender string
 * "method.". The remaining part of the string is the name of the method
 * to execute, e.g., pushing the first button will execute the findName
 * method and the second button will execute the findEmail method.
 * </p>
 * 
 * <p>
 * This class extends DispatchAction and allows methods to be sent as
 * regular GETs as well, i.e., &lt;a
 * href="emailAction.do?method=findEmail"/&gt; would cause the findEmail
 * method to be executed just as it would in a DispatchAction. Thus, you
 * configure a ButtonNameDispatchAction exactly the way you configure a
 * DispatchAction, i.e., set the mapping parameter to the name of the
 * request parameter that holds the mehtod name.
 * </p>
 * 
 * <p>
...
public class BaseAction extends DispatchAction {
  ...
  static {
    ...
    ConvertUtils.register(new NullConverter(), FormFile.class);
  }
  ...
  /**
   * @see gleam.executive.util.ConvertUtil#convert(java.lang.Object)
   */
  ...
  /**
   * @see gleam.executive.util.ConvertUtil#convertLists(java.lang.Object)
   */
  ...
  protected String getActionMethod(HttpServletRequest request, String prepend) {
    ...
    if(name != null) {
      ...
      return name
              .replace(name.charAt(0), Character.toLowerCase(name.charAt(0)));
    }
  ...
  /**
   * Override the execute method in DispatchAction to parse URLs and
   * forward to methods without parameters.
   * </p>
   * <p>
  ...
  /**
   * Convenience method for getting an action form base on it's mapped
   * scope.
   * 
   * @param mapping The ActionMapping used to select this instance
   * @param request The HTTP request we are processing
   * @return ActionForm the form from the specifies scope, or null if
   *         nothing found
   */
  ...
  /**
   * Convenience method to get the Configuration HashMap from the
   * servlet context.
   * 
