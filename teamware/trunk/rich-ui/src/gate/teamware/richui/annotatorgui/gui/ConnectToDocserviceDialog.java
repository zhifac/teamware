/*
 *  ConnectToDocserviceDialog.java
 *
	...
	protected void connect() {
		...
		if (annotationSetName.getSelectedItem().equals(
			...
			connectAsync(this.docserviceUrl.getText().trim(), this.ontologyUrl
					...
					this.owlimRepositoryName.getText(), this.documentId
							.getText(), null);
		} else {
			connectAsync(this.docserviceUrl.getText().trim(), this.ontologyUrl
					.getText().trim(), this.owlimServiceURL.getText(),
					this.owlimRepositoryName.getText(), this.documentId
							.getText(), (String) annotationSetName
							.getSelectedItem());
		}
	}
		...
		Runnable connectRun = new Runnable() {
			public void run() {
				...
				try {
					...
					// load an ontology if either (a) service and repository are specified
					// or (b) ontology ontologyURL is specified.
					boolean goAhead = false;
					if ((owlimServiceURL != null && owlimServiceURL.trim().length() > 0
							&& owlimRepositoryName != null && owlimRepositoryName.trim().length() > 0)
						|| (ontologyUrlString != null && ontologyUrlString.trim().length() > 0)) {
						goAhead = true;
					}
					...
					if (goAhead) {
						...
						try {
							...
							handleError(new RichUIException(
									"Filed to load ontology. Unexpected error:\n\n"
											+ e.getClass().getName()
											+ " occured. Message:\n"
											+ e.getMessage(), e));
						}
