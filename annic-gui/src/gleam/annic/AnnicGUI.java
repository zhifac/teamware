...
import gleam.annic.actions.ActionConnectToAnnicGUI;
...
public class AnnicGUI implements Constants {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < args.length; i++) {
			...
			if (arg.trim().startsWith(DEBUG_PARAMETER_NAME + "=")) {
				...
				properties.put(CORPUS_ID_PARAMETER_NAME, arg.trim().substring(
						CORPUS_ID_PARAMETER_NAME.length() + 1).trim());
			}
			// ===
			else {
		...
		if (properties.getProperty(AUTOCONNECT_PARAMETER_NAME, "")
			...
			if (properties.getProperty(CORPUS_ID_PARAMETER_NAME) == null
					|| properties.getProperty(CORPUS_ID_PARAMETER_NAME)
							.length() == 0) {
				
				// if no corpus ID is provided
				String err = "Can't autoconnect. A parameter '"
						+ CORPUS_ID_PARAMETER_NAME + "' is missing.";
				System.out.println(err);
				JOptionPane.showMessageDialog(MainFrame.getInstance(), err,
						"Error!", JOptionPane.ERROR_MESSAGE);
				
				// then we invoke the annic connection dialog
				ActionShowAnnicConnectDialog.getInstance()
						.actionPerformed(null);
			} else {
				// else we connect directlry
				ActionConnectToAnnicGUI.getInstance().actionPerformed(null);
			}
		} else {
			// if it is set not to start directly, we show the connection dialog
			ActionShowAnnicConnectDialog.getInstance().actionPerformed(
					null);
		}
	}
	...
	public static void setConnection(Connection newConnection)
			throws AnnicGUIExeption {
		System.out.println("Annic GUI set connection "
				+ newConnection.toString());
		
		// if there's already a set connect, we close it first
		if (AnnicGUI.getConnection() != null) {
			closeConnection();
		}
		AnnicGUI.connection = newConnection;
		System.out.println("Annic GUI status of connection "
				+ getConnectionStatus());
		// here we update all status messages
		MainFrame.getInstance().updateAllStatuses();
		// fire this event to update our gui
	...
	public static boolean closeConnection() throws AnnicGUIExeption {
		if (connection == null)
			return true;
		connection.cleanup();
		connection = null;
	...
	public static void loadPlugins(String plugins, boolean unloadOtherPlugins) {
		// this method would never be invovked as there are no plugins required for annic
		// however, we keep it here in case we need it in future
		if (plugins == null || plugins.length() == 0) {
			System.out.println("No plugins specified");
			return;
		}
		
		// plugins string has names of plugins delimited with a comma in between them
		...
		while (stok.hasMoreTokens()) {
			...
			if (tok.length() > 0) {
				...
				URL url = AnnicGUI.class.getClassLoader().getResource(
						tok + "/creole.xml");
				if (url == null) {
