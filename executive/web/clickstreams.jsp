...
while (it.hasNext())
{
    ...
    if(stream.getHostname() != null && !stream.getHostname().equals("")){ 
    	count++; %>
    
<%= count %>. <u><%= stream.getUsername()%></u> : <a href="viewstream.jsp?sid=<%= key %>"><b><%= stream.getHostname()%></b></a> [<%= stream.getStream().size() %> reqs]<br />
