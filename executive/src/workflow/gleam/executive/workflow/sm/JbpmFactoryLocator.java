...
/**
 * BeanFactoryLocator used for injecting Spring application context into
 * JBPM. The difference/advantage over the traditional
 * SingletonBeanFactoryLocator is that it does not parse a bean factory
 * definition; it is used internally by the jbpmSessionFactoryBean and
 * it will register the bean factory/application context containing it
 * automatically under the name and and aliases of the bean. If there is
 * only one BeanFactory registered then a null value can be used with
 * setBeanName method. <p/> Note that in most cases, you don't have to
...
public class JbpmFactoryLocator implements BeanFactoryLocator,
  ...
  public void setBeanFactory(final BeanFactory beanFactory)
    ...
    synchronized(JbpmFactoryLocator.class) {
      if(canUseDefaultBeanFactory) {
        if(defaultFactory == null) {
          ...
          if(logger.isDebugEnabled())
            logger
                    .debug("more then one beanFactory - default not possible to determine");
    ...
    if(counter == null) referenceCounter.put(beanFactory, new Integer(0));
    // add aliases
    ...
    if(previousNames != null) names.addAll(previousNames);
  ...
  protected void addToMap(String fName, BeanFactory factory) {
    ...
    synchronized(beanFactories) {
      // override check
      // commented by agaton due to test problems
      /*
       * if (beanFactories.containsKey(fName)) throw new
       * IllegalArgumentException("a beanFactoryReference already exists
       * for key " + factoryName);
       */
  ...
  protected void removeReference(BeanFactory factory) {
    synchronized(referenceCounter) {
      ...
      else referenceCounter.put(factory, new Integer(counter));
    }
  }
  ...
  public BeanFactoryReference useBeanFactory(String factoryKey)
    ...
    if(factoryKey == null) {
      ...
      if(factory == null)
        throw new IllegalArgumentException("there is no beanFactory under key "
                + factoryKey);
    }
    // increment counter
    synchronized(referenceCounter) {
