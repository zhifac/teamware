 ...
 * 
 * @jsp.tag name="customURL" bodycontent="JSP"
 * 
 */

public class CustomURLTag extends BodyTagSupport {

	private static Log log = LogFactory.getLog(CustomURLTag.class);
		...
		try {
			...
			if (parameters != null && parameters.length != 0) {
				for (int i = 0; i < parameters.length; i++) {
					...
					// look for a variable only if starts with caps -> that is a sign it in task instance scope
					if (Character.isUpperCase(parameterValues[i].charAt(0)) &&  
					    workflowManager.findVariable(new Long(task),parameterValues[i]) != null) {
		 ...
		 WebAppBean webAppBean = (WebAppBean)ctx.getBean("webAppBean");
			...
			if(url.contains("popup=true")){
			    link = "<a title=\"[Open popup window]\" onclick=\"popupBox('" 
					+ url + "')\"  href=\"#\">" + this.name + "</a>";
			    log.debug("constructed popup link: "+ link);
			}
			else {
				link = "<a title=\""+ this.name + "\" href=\""+url + "\">" + this.name + "</a>";
				log.debug("constructed normal link: "+ link);
			}
