...
public class DocServiceDataStore extends AbstractFeatureBearer implements DataStore, CreoleListener {
  ...
  private JaxWsProxyFactoryBean getFactoryBean() {
	...
	public LanguageResource adopt(LanguageResource lr, SecurityInfo secInfo) throws PersistenceException,
			SecurityException {
		long t = System.currentTimeMillis();
		if (DEBUG)
			System.out.println("DEBUG: " + this.getClass().getName() + ".adopt()" + " lr.name=" + lr.getName() + " secInfo='"
					+ secInfo + "'");
		...
		try {
			...
			if (PROFILE)
				System.out.println("PROFILE: " + this.getClass().getName() + ".adopt()" + " lr.name=" + lr.getName()
						+ " Resource adopted in " + (System.currentTimeMillis() - t) + " ms");
			return res;
	...
	public void delete(String lrClassName, Object lrId) throws PersistenceException, SecurityException {
		if (DEBUG)
			System.out.println("DEBUG: " + this.getClass().getName() + ".delete(String, Object), lrClassName='" + lrClassName
					+ "' lrId='" + lrId + "'");
		try {
			if (DOCSERVICE_CORPUS_CLASS_NAME.equals(lrClassName)) {
				...
				// removeDocumentLocks((String) lrId, true);
			}
	...
	public LanguageResource getLr(String lrClassName, final Object lrId) throws PersistenceException, SecurityException {
		long t = System.currentTimeMillis();
		if (DEBUG)
			System.out.println("DEBUG: " + this.getClass().getName() + ".getLr()" + " lrClassName='" + lrClassName + "'"
					+ " lrId='" + lrId + "'");
		if (!(lrId instanceof String)) {
			throw new PersistenceException("" + this.getClass().getName()
					+ " only supports String type of Language Resource IDs");
		}
		...
		try {
			LanguageResource res;
			if (DOCSERVICE_CORPUS_CLASS_NAME.equals(lrClassName)) {
				if (this.sds.getCorpusInfo((String) lrId) == null)
					throw new PersistenceException("Cant get corpus '" + lrId + "' from datastore.");
				FeatureMap fm = Factory.newFeatureMap();
        ...
        res.setFeatures(corpusFeatures);
				...
				if (PROFILE)
					System.out.println("PROFILE: " + this.getClass().getName() + ".getLr(" + lrClassName + ", " + lrId
							+ ") getDocContent() content.length=" + docContent.length() + " takes "
							+ (System.currentTimeMillis() - t1) + " ms");
				long t2 = System.currentTimeMillis();
				FeatureMap fm = Factory.newFeatureMap();
				fm.put("stringContent", docContent);
				res = (gate.corpora.DocumentImpl) Factory.createResource(DOCUMENT_CLASS_NAME, fm, null, this.sds.getDocInfo(
						(String) lrId).getDocumentName());
				if (PROFILE)
					System.out.println("PROFILE: " + this.getClass().getName() + ".getLr(): Factory.createResource() takes "
        ...
        res.setFeatures(docFeatures);
				//if (DEBUG)
				//	System.out.println("DEBUG: " + this.getClass().getName() + ".getLr(): getting default annotation set...");
				//long t3 = System.currentTimeMillis();
				//AnnotationSetHandle ash = this.sds.getAnnotationSet((String) lrId, null, false);
				//if (ash.getTaskID() == null || ash.getTaskID().length() == 0) {
				//	throw new PersistenceException("Invalid task ID for document's default annotation set. Task ID="
				//			+ ash.getTaskID() + ". Document ID=" + lrId);
				//}
				//String s = ash.getData();
				//if (PROFILE)
				//	System.out.println("PROFILE: " + this.getClass().getName()
				//			+ ".getLr() get-default-annotation-set: xmldata.length=" + s.length() + " takes "
				//			+ (System.currentTimeMillis() - t3) + " ms");
				//DocserviceUtil.setAnnotationSet(((Document) res).getAnnotations(), s);
				//docLocks.put(null, ash.getTaskID());
				//Thread notifier = createTaskNotifier(this.sds, ash.getTaskID());
				//notifier.start();
				//docNotifiers.put(null, notifier);
				long t4 = System.currentTimeMillis();
				// listAnnotationSets is never empty as it always contains at least null (default set)
				String[] asNames = this.sds.listAnnotationSets((String) lrId);
				if (PROFILE)
					System.out.println("PROFILE: " + this.getClass().getName() + ".getLr(): listAnnotationSets() takes "
							+ (System.currentTimeMillis() - t4) + " ms");
				for (int i = 0; i < asNames.length; i++) {
					if (DEBUG)
						System.out.println("DEBUG: " + this.getClass().getName() + ".getLr(): getting named annotation set '"
								+ asNames[i] + "'...");
					long t5 = System.currentTimeMillis();
					AnnotationSetHandle ash = this.sds.getAnnotationSet((String) lrId, asNames[i], false);
					if (ash.getTaskID() == null || ash.getTaskID().length() == 0) {
						throw new PersistenceException("Invalid task ID for document's named annotation set '" + asNames[i]
								+ "'. Task ID=" + ash.getTaskID() + ". Document ID=" + lrId);
					}
					byte[] s = ash.getData();
					if (PROFILE)
						System.out.println("PROFILE: " + this.getClass().getName() + ".getLr(): get-annotation-set '" + asNames[i]
								+ "' xmldata.length=" + s.length + " takes " + (System.currentTimeMillis() - t5) + " ms");
				...
				throw new PersistenceException("Language Resources of type '" + lrClassName
						+ "' not supprted by DocServiceDataStore.");
			}
			if (PROFILE)
				System.out.println("PROFILE: -------------------- " + this.getClass().getName() + ".getLr(" + lrClassName
						+ ", " + lrId + ") Resource loaded in " + (System.currentTimeMillis() - t) + " ms");
			return res;
  ...
  public String getLrName(Object lrId) {
	...
	/**
	 * TODO: This method present a problem if it called to often. Visual resource
	 * call this method many times to render a tree. 
	 */
/*
  public String getLrName(Object lrId) throws PersistenceException {
		if (DEBUG) System.out.println("DEBUG: " + this.getClass().getName() + ".getLrName() lrId='" + lrId + "'");
		CorpusInfo[] CIs;
		try {
			CIs = sds.listCorpora();
			if (CIs.length > 0) {
				for (int i = 0; i < CIs.length; i++) {
					if (CIs[i].getCorpusID().equals(lrId)) return CIs[i].getCorpusName();
				}
			}
			DocumentInfo[] DIs = sds.listDocs();
			if (DIs.length > 0) {
				for (int i = 0; i < DIs.length; i++) {
					if (DIs[i].getDocumentID().equals(lrId)) return DIs[i].getDocumentName();
				}
			}
			return null;
...
*/
	...
	public void open() throws PersistenceException {
	    ...
	    Thread.currentThread().setContextClassLoader(Gate.getClassLoader());
		  ...
		  JaxWsProxyFactoryBean factoryBean = getFactoryBean();
			...
			// this.sds = this.sdss.getdocservice(new URL(this.storageUrl));
	...
	public void setStorageUrl(String storageUrl) throws PersistenceException {
		if (DEBUG)
			System.out.println("DEBUG: " + this.getClass().getName() + ".setStorageUrl()" + " current storageUrl='"
	...
	public void sync(LanguageResource lr) throws PersistenceException, SecurityException {
		long t = System.currentTimeMillis();
		if (DEBUG)
			System.out.println("DEBUG: " + this.getClass().getName() + ".sync()" + " lr.name=" + lr.getName() + " ID='"
					+ lr.getLRPersistenceId() + "'");
		...
		try {
			if (lr instanceof Document) {
				...
				// check if the corpus is the one we support. CorpusImpl cannot be
				// saved!
				if (!(lr instanceof DocServiceCorpusImpl))
					throw new PersistenceException("Can't save a corpus which " + "is not of type DocServiceCorpusImpl!");
			...
			if (PROFILE)
				System.out.println("PROFILE: -------------------- " + this.getClass().getName() + ".sync()" + " lr.name="
						+ lr.getName() + " ID='" + lr.getLRPersistenceId() + "' Resource synchronized in "
	...
	public String getName() {
		if (DEBUG)
			System.out.println("DEBUG: " + this.getClass().getName() + ".getName() return value='" + this.name + "'");
		return this.name;
	}

	public void setName(String name) {
		if (DEBUG)
			System.out.println("DEBUG: " + this.getClass().getName() + ".setName()" + " current name='" + this.name + "'"
					+ " new name='" + name + "'");
		this.name = name;
	}
	...
	private void fixCRLF(Document doc) {
		...
		if (DEBUG && i != -1) {
			System.out.println("DEBUG: " + this.getClass().getName()
					+ ".sync(): local doc content content contains on or more \"/r/n\" which need to be fixed");
		}
		if (DEBUG_DETAILS && i != -1) {
			System.out.println("DEBUG_DETAILS: " + this.getClass().getName() + ".sync(): local doc content: '"
		...
		if (DEBUG_DETAILS) {
			System.out.println("DEBUG_DETAILS: " + this.getClass().getName() + ".adopt: local doc fixed content: '"
					+ doc.getContent().toString().replaceAll("\n", "/n\n").replaceAll("\r", "/r\r") + "'");
		}
		if (PROFILE)
			System.out.println("PROFILE: " + this.getClass().getName() + ".fixCRLF()" + " docName=" + doc.getName()
					+ " fixCRLF() takes " + (System.currentTimeMillis() - t) + " ms");
	}
	...
	private void checkDocContent(Document doc) throws PersistenceException {
		...
		if (!remoteDocText.equals(doc.getContent().toString())) {
			if (DEBUG) {
				System.out.println("DEBUG: " + this.getClass().getName() + ".checkDocContent(): local doc content: '"
						+ doc.getContent().toString().replaceAll("\n", "/n\n").replaceAll("\r", "/r\r") + "'");
				System.out.println("DEBUG: " + this.getClass().getName() + ".checkDocContent(): remote doc content: '"
						+ remoteDocText.replaceAll("\n", "/n\n").replaceAll("\r", "/r\r") + "'");
			}
			throw new PersistenceException(
					"Can't synchronize document. Document texts are different at the server and client sides.");
		}
		if (PROFILE)
			System.out.println("PROFILE: " + this.getClass().getName() + ".checkDocContent()" + " docName=" + doc.getName()
					+ " checkDocContent() takes " + (System.currentTimeMillis() - t) + " ms");
	}
	...
	private void saveDocument(Document doc) throws RemoteException {
		...
		try {
			...
			if (locksForDelete.containsKey(null)) {
				...
				if (DEBUG)
					System.out.println("DEBUG: " + this.getClass().getName()
							+ ".saveDocument(): getting default annotation set in RW mode...");
			...
			if (doc.getNamedAnnotationSets() != null) {
				...
				while (itr.hasNext()) {
					...
					if (locksForDelete.containsKey(asName)) {
						...
						if (DEBUG)
							System.out.println("DEBUG: " + this.getClass().getName()
									+ ".saveDocument(): getting named annotation set '" + asName + "' in RW mode...");
						locksForUpdate.put(asName, this.sds.getAnnotationSet((String) doc.getLRPersistenceId(), asName, false)
								.getTaskID());
					}
				}
			}
			...
			while (itr.hasNext()) {
				...
				if (DEBUG)
					System.out.println("DEBUG: " + this.getClass().getName() + ".saveDocument(): updating annotation set '"
							+ asName + "' taskId='" + taskId + "'");
			...
			while (itr.hasNext()) {
				...
				String taskId = (String) entry.getValue();
				if (DEBUG)
					System.out.println("DEBUG: " + this.getClass().getName() + ".saveDocument(): deleting annotation set '"
							+ asName + "' taskId='" + taskId + "'");
      ...
      this.sds.setDocumentFeatures((String) doc.getLRPersistenceId(), MapWrapper.wrap(m));
			...
			if (PROFILE)
				System.out.println("PROFILE: " + this.getClass().getName() + ".saveDocument()" + " docName=" + doc.getName()
						+ " saveDocument() takes " + (System.currentTimeMillis() - t) + " ms");
		} catch (Exception e) {
			e.printStackTrace();
			Iterator itr = locksForUpdate.entrySet().iterator();
			while (itr.hasNext()) {
				...
				String taskId = (String) entry.getValue();
				if (DEBUG)
					System.out.println("DEBUG: " + this.getClass().getName() + ".saveDocument(): releasing 'update' lock '"
							+ taskId + "' for annotation set '" + asName + "'");
			...
			while (itr.hasNext()) {
				...
				String taskId = (String) entry.getValue();
				if (DEBUG)
					System.out.println("DEBUG: " + this.getClass().getName() + ".saveDocument(): releasing 'delete' lock '"
							+ taskId + "' for annotation set '" + asName + "'");
				this.sds.releaseLock((String) entry.getKey());
			}
		}
	...
	private Map getAllLocks(String docId) throws DocServiceException, RemoteException {
		...
		if (PROFILE)
			System.out.println("PROFILE: " + this.getClass().getName() + ".getAllLocks()" + " docID='" + docId
					+ " getAllLocks() takes " + (System.currentTimeMillis() - t) + " ms");
		return docLocks;
	}
	...
	private void saveCorpus(DocServiceCorpusImpl corpus) throws DocServiceException, PersistenceException, SecurityException {
		...
		if (corpus.getLRPersistenceId() == null) {
			if (DEBUG)
				System.out.println("DEBUG: " + this.getClass().getName()
						+ ".saveCorpus(): this corpus doesn't contain persistent ID, so creating new corpus in datastore...");
    ...
    this.sds.setCorpusFeatures((String) corpus.getLRPersistenceId(), MapWrapper.wrap(m));
		...
		if (PROFILE)
			System.out.println("PROFILE: " + this.getClass().getName() + ".saveCorpus()" + " corpusName=" + corpus.getName()
					+ " saveCorpus() takes " + (System.currentTimeMillis() - t) + " ms");
	}

	private void removeNotifiers(Map docNotifiers, String docID) {
		if (DEBUG)
			System.out.println("DEBUG: " + this.getClass().getName() + ".removeNotifiers(): removing notifiers for docID="
					+ docID);
	...
	private void releaseDocLocks(Map docLocks, String docID) {
		if (DEBUG)
			System.out.println("DEBUG: " + this.getClass().getName() + ".releaseDocLocks(): releasing locks for docID="
					+ docID);
